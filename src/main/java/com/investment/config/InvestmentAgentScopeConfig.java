 ...
 */
@Configuration
public class InvestmentAgentScopeConfig {
    ...
    @Value("${investment.llm.main-model:qwen-max}")
    private String mainModelName;

    /** 轻量模型用于意图识别和澄清追问，默认使用 qwen-turbo。 */
    @Value("${investment.llm.light-model:qwen-turbo}")
     ...
     */
    @Bean("InvestmentMainChatModel")
    public Model InvestmentMainChatModel() {
     ...
     */
    @Bean("InvestmentLightChatModel")
    public Model InvestmentLightChatModel() {
        return DashScopeChatModel.builder()
                .apiKey(apiKey)
