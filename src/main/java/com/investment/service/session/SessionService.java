...
import com.investment.model.SessionMessageRow;
...
/**
 * 会话消息落库服务。
 * 负责 investment_sessions 创建和 investment_messages 追加；会话状态（slots/phase）由 SessionStateService 管理。
 */
@Service
public class SessionService {

    /** MyBatis Mapper，操作 investment_sessions 和 investment_messages 表。 */
    private final SessionMapper sessionMapper;

    /** JSON 序列化工具。 */
    private final JsonService jsonService;

    /** 注入 IntentAgent 的最近对话条数上限，来自配置 investment.session.max-history-turns。 */
    ...
    public SessionService(
            ...
            @Value("${investment.session.max-history-turns:10}") int maxHistoryTurns
    ) {
    ...
    public String createSession(Long userId) {
        ...
        sessionMapper.insert(row);                                           // INSERT investment_sessions
    ...
    /**
     * 追加一条对话消息到 investment_messages 表。
