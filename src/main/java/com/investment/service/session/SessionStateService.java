...
/**
 * 会话状态读写服务。
 * Orchestrator 是唯一写 SessionState 的角色；investmentChat 链路中 loadOrCreate 和 save 被频繁调用。
 */
@Service
public class SessionStateService {
    ...
    /**
     * MyBatis Mapper，读写 investment_sessions 表。
     */
    ...
    public SessionState create(Long userId, SourceMode sourceMode) {
        ...
        insert(state);                                                               // INSERT investment_sessions
        return state;
    }

    /**
     * 加载或创建会话状态。
     * 由 Orchestrator#investmentChat 入口调用，返回 slots/phase/lastRecommendations 等。
     */
    public SessionState loadOrCreate(String sessionId, Long userId, SourceMode sourceMode) {
        ...
        // 按 sessionId + userId 查 investment_sessions（校验归属）
    ...
    public void save(SessionState state) {
        ...
        int updated = sessionMapper.update(row);    // UPDATE investment_sessions
        if (updated == 0) {
            throw new InvestmentException("会话状态保存失败"); // 影响行数 0 说明 sessionId 不存在或并发冲突
        }
    }

    /**
     * INSERT 新会话行到 investment_sessions。
     */
    ...
    /**
     * 将 investment_sessions 行反序列化为 SessionState 业务对象。
     */
    private SessionState fromRow(SessionRow row, SourceMode requestSourceMode) {
        try {
            ...
            SlotBundle slots = new SlotBundle(
                    readStringList(root, "investmentAmount"),
                    readStringList(root, "investmentHorizon"),
                    readStringList(root, "riskPreference"),
                    readStringList(root, "liquidityNeed"),
                    readStringList(root, "returnExpectation"),
                    readStringList(root, "productType"),
                    readStringList(root, "customerProfile"),
                    readStringList(root, "restriction")
            );
            ...
            throw new InvestmentException("会话状态解析失败", e);
        }
    }
    ...
    /**
     * 将投资槽位 + _meta（sourceMode/currentIntent）序列化为 slots 列 JSON。
     */
    private String toSlotsJson(SessionState state) {
        ObjectNode root = objectMapper.createObjectNode();
        root.set("investmentAmount", objectMapper.valueToTree(state.slots().investmentAmount()));
        root.set("investmentHorizon", objectMapper.valueToTree(state.slots().investmentHorizon()));
        root.set("riskPreference", objectMapper.valueToTree(state.slots().riskPreference()));
        root.set("liquidityNeed", objectMapper.valueToTree(state.slots().liquidityNeed()));
        root.set("returnExpectation", objectMapper.valueToTree(state.slots().returnExpectation()));
        root.set("productType", objectMapper.valueToTree(state.slots().productType()));
        root.set("customerProfile", objectMapper.valueToTree(state.slots().customerProfile()));
        root.set("restriction", objectMapper.valueToTree(state.slots().restriction()));
    ...
    private String toJson(Object value) {
        try {
            ...
            throw new InvestmentException("会话状态 JSON 序列化失败", e);
        }
    }
